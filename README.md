...
### 类的本质
有些类是本职类，就是这个类是做相关工作的。
有些类是沟通类，负责衔接各个本质类之间的联系。
...
微信原生的button实现分享功能，需要**设置open-type属性的值为share**，就可以触发分享功能。
...
因为微信的分享功能是通过原生的button组件实现的，但是想要修改原生的button按钮的样式是非常麻烦和困难的，所以不建议使用这种方式。

##### 方式2：button+view

我们在下面使用微信原生的button实现实际的分享功能，然后使用定位的方式，在button按钮上方自定义一个好看的view来模拟按钮，这样即实现了美观的样式，也实现了分享功能。

##### 方式三3：使用组件库

本项目中使用的lin-ui组件库，的按钮其实本质上也是view实现的。但是也具备微信的开发能力，可以实现分享功能。
